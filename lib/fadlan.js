...
class fadlanNEW {
  GambarJadiStiker(jid, stickerData, quot, options = {}) {
    (async () => {
      var packname = options.pack || "F4D4L4N";
      var author = options.author || "SELF";
      convertSticker(stickerData, author, packname)
        .then(bsres => {
          var imageBuffer = new Buffer.from(bsres, "base64");
          fadlan.sendMessage(jid, imageBuffer, MessageType.sticker, {
            quoted: quot
          });
        })
        .catch(err => {
          console.log(err);
          fadlan.sendMessage(jid, "error", MessageType.text, {
            quoted: quot
          });
        });
    })();
  ...
  FakeStatus(to, teks, fake){
            fadlan.sendMessage(to, teks, MessageType.text, {
		contextInfo: { 					
                ...
                quoted: {
                    ...
                    message: {
                        "imageMessage": {
                            ...
                            "jpegThumbnail": fs.readFileSync('./src/fadlan.jpg'),
		...
		FakeThumb(to, teks, quot, capt){
            fadlan.sendMessage(to, teks, MessageType.image, {thumbnail:fs.readFileSync('./src/fadlan.jpg'),quoted:quot,caption:capt})
        }
		...
		FileDariUrl(to, url, quot, capt=""){
                ...
                download(url, filename, async function () {
                    ...
                    fadlan.sendMessage(to, media, type, { quoted: quot, mimetype: mime, caption: capt,contextInfo: {"mentionedJid": sender}})
                    
                    fs.unlinkSync(filename)
                });
            } 
}
global.Kirim = new fadlanNEW();
module.exports = "GASS!!!";

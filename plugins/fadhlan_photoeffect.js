			...
			switch(command){
				...
				case 'beautiful':
                    if ((isMedia && !msg.message.videoMessage || isQuotedImage)) {
                        ...
                        file_fadlan = await fadlan.downloadMediaMessage(encmedia);
                        var req = request({
							...
							fadlan.sendMessage(from, gas, MessageType.image, {quoted: msg, caption: 'Ini kak :v'})
                        });
						let form = req.form();
						form.append('img', file_fadlan, { filename : 'gas.png' })
                    } else {
				...
				case 'mintframe':
					...
					.then(async (gaya) => {
						fadlan.sendMessage(from, gaya.data, MessageType.image, {quoted: msg, caption: 'Ini kak :v'})
					})
                    } else {
                        reply(`Kirim gambar dengan caption ${prefix + command} atau tag gambar yang sudah dikirim`)
                    }
